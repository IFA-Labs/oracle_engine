...
func (ds *DataStream) Start(ctx context.Context, cfg *config.Config) {
	for _, asset := range cfg.Assets {
		for _, feedCfg := range asset.Feeds {
			...
			go ds.runFeed(ctx, asset.Name, feedAssetID, feed, time.Duration(feedCfg.Interval)*time.Second)
		}
	}
}

func (ds *DataStream) runFeed(ctx context.Context, asset string, assetId string, feed PriceFeed, interval time.Duration) {
	...
	for {
		...
		case <-ctx.Done():
			return
		case <-ticker.C:
			price, err := feed.FetchPrice(ctx, assetId)
			if err != nil {
